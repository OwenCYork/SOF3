...
plus pos move = doMove pos move
 where
  doMove pos move | move == 0 = pos
                  | pos == Home = Home
                  | otherwise = doMove (succ pos) (move-1)
test_plus :: Bool
...
toList place = [((x,y),i) | x <- [Start .. Home], y <- [Red,Green], i <- [place (x,y)]]
fromList list = doFromList list zeros
 where
  doFromList :: [((Position,Player),Int)] -> Placement -> Placement
  doFromList [] plac = plac
  doFromList (x@((pos,player),i):xs) plac = doFromList xs (add plac pos player i)
  zeros :: Placement
  zeros (_,_) = 0
  add :: Placement -> Position -> Player -> Int -> Placement
  add plac pos player i (p,pl) | pos==p && player==pl = i
                               | otherwise = plac (p,pl)
testToFromList :: Bool
