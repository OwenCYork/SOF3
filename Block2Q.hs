...
oneone' n = doOneone n
  where
  doOneone [] = []
  doOneone (n:ns) | n `mod` 2 == 0 = n+1:doOneone ns
                  | n `mod` 2 == 1 = doOneone ns
                  | otherwise = []

oneoneTest = oneone' [1,2,4,5,6] == [3,5,7]
...
onetwo' c = undefined
  -- where
  -- doOneTwo [] = []
  -- doOneTwo (c:cs) = concat (map fromEnum c : doOneTwo cs)
...
--[True, False, True, False] returns 10
--[True, False, False, False] gives an exeception, non-exaustive.
...
ePbs2iM bs | parity bs = bitstring2int bs
           | otherwise = Nothing
{-
