...
checkMarks students = checkStudents students
 where
  checkStudents [] = True
...
checkPrereqs stud = doCheckPrereqs stud (prereqs stud [])
 where

  prereqs (Student x) p = formPrereq x p
  formPrereq :: [(Module, Maybe Int)] -> [Module] -> [Module]
  formPrereq [] p = p
