...
cakeBill' quantity price
 | quantity == 1 = "The cost of " ++ (show quantity) ++ " cake at " ++ (show price) ++ "p each is " ++ (show (quantity*price)) ++ "p."
...
bananas :: Int -> Int
bananas order | order < min_order = error "Order is less than 2kg"
              | (order*300)+499 > 5000 = (order*300) + 349
...
implies'' True False = False
implies'' _ _ = True -- using "don't care" patterns
...
-}
greetTest' = map gt [("Kofi", "Hello Kofi!"),
            ("Jeremy", "Hello Jeremy!"),
            ("", "Hello !")]
  where
   gt (testInput,expectedOutput) = greet testInput == expectedOutput
...
pos :: Eq a => a -> [a] -> Int
pos x n = findPos 0 x n
 where
  findPos count x (n:ns) 
   | n == x = count
   | otherwise = findPos (count + 1) x ns 
...
-}
insert :: Ord a => a -> [a] -> [a]
insert x orderedList = doInsert orderedList
  where
   doInsert [] = [x]
   doInsert (n:ns) | x > n = n:(doInsert ns)
                   | x <= n = x:n:ns
{-
