...
-}
fPlayer :: IO ()  
fPlayer = do putStr "Who wants to play first: "
             p <- getLine
             case p of 
               "O" -> run empty O
               "X" -> run empty X
               cs -> do putStr "INVALID input\n"
                        fPlayer
...
winner :: String -> IO ()
winner x = do writeFile "champion.txt" (if x /= "X" && x /= "O" then "" else x)
...
fPlayer' :: IO ()  
fPlayer' = do lastwin <- readFile "champion.txt"
              putStr "Who wants to play first: "
              p <- getLine
              case p of 
                "O" -> run empty O
                "X" -> run empty X
